...
export function GeneralRegisterEditForm({ 
  ...
  const [formData, setFormData] = useState({
    ...
    dueDateDays: null as number | null,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <Select
          ...
          onChange={(e) => {
            ...
            setFormData(prev => ({ 
              ...prev, 
              solutionStatus: value === 'in_work' ? null : (value ? (value as 'approved' | 'rejected' | 'redirected') : null)
            }));
          }}
          options={[
            ...
            { value: 'in_work', label: 'Lăsare în lucru' },
          ]}
      ...
      <div>
        <Input
          ...
          value={formData.dueDate}
            onChange={(e) => {
              ...
              handleFieldChange('dueDate', dateValue);
              
              // Calculate days from today if date is set
              if (dateValue) {
                const today = new Date();
                const dueDate = new Date(dateValue);
                const diffTime = dueDate.getTime() - today.getTime();
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                setFormData(prev => ({ ...prev, dueDateDays: diffDays >= 0 ? diffDays : null }));
              } else {
                setFormData(prev => ({ ...prev, dueDateDays: null }));
              }
            }}
          />
        </div>
        <div>
          <Input
            label="Termen (Număr zile)"
            type="number"
            min="0"
            value={formData.dueDateDays?.toString() || ''}
            onChange={(e) => {
              const daysValue = e.target.value;
              const days = daysValue ? parseInt(daysValue, 10) : null;
              setFormData(prev => ({ ...prev, dueDateDays: days }));
              
              // Calculate date from days if days is set
              if (days !== null && days >= 0) {
                const today = new Date();
                const dueDate = new Date(today);
            ...
            placeholder="Număr de zile de la astăzi"
          />
        </div>
      </div>
