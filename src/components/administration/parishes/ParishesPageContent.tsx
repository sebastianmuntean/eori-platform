...
interface ParishesPageContentProps {
  locale: string;
}

/**
 * Parishes page content component
 * Contains all the JSX/HTML that was previously in the page file
 * Separates presentation from routing and permission logic
 */
export function ParishesPageContent({ locale }: ParishesPageContentProps) {
  ...
  const columns = [
    { key: 'code' as keyof Parish, label: t('code') || 'Code', sortable: true },
    { key: 'name' as keyof Parish, label: t('name') || 'Name', sortable: true },
    { key: 'city' as keyof Parish, label: t('city') || 'City', sortable: true },
    { key: 'county' as keyof Parish, label: t('county') || 'County', sortable: true },
    {
      ...
      render: (_: any, row: Parish) => (
        <Dropdown
          ...
          items={[
            ...
            { label: t('delete') || 'Delete', onClick: () => handleDelete(row.id), variant: 'danger' },
          ]}
          align="right"
        />
      ),
