...
interface PilgrimagesPageContentProps {
  locale: string;
}

/**
 * Pilgrimages page content component
 * Contains all the JSX/HTML that was previously in the page file
 * Separates presentation from routing and permission logic
 */
export function PilgrimagesPageContent({ locale }: PilgrimagesPageContentProps) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, parishFilter, statusFilter, dateFrom, dateTo]);

  // Fetch pilgrimages with current filters and pagination
  useEffect(() => {
    const params = {
      page: currentPage,
      pageSize: 10,
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        sortable: true,
        render: (value: string | null) => formatDateLocalized(value),
      },
      {
    ...
    [tPilgrimages, formatDateLocalized, getStatusLabel, t, locale, router, handleEdit, approvePilgrimage, publishPilgrimage, closePilgrimage, cancelPilgrimage]
  );
