...
interface ReportPageWithCRUDProps {
  title: string;
  titleKey: string;
  href: string;
  filterParams?: {
 ...
 */
export function ReportPageWithCRUD({ title, titleKey, href, filterParams = {} }: ReportPageWithCRUDProps) {
  ...
  }, [currentPage, searchTerm, parishFilter, fetchFixedAssets]);
  ...
  const resetForm = () => {
    setFormData({
      ...
      depreciationMethod: '',
      usefulLifeYears: '',
      status: filterParams.status as any || 'active',
      disposalDate: '',
      disposalValue: '',
      disposalReason: '',
      notes: '',
  ...
  const handleSave = async () => {
    if (selectedAsset) {
      ...
      if (result) {
        ...
        setSelectedAsset(null);
        resetForm();
        fetchFixedAssets({ page: currentPage, pageSize: 10, ...filterParams });
      }
    } else {
