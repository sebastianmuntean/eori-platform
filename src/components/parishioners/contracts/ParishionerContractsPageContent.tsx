...
interface ParishionerContractsPageContentProps {
  locale: string;
}

/**
 * Parishioner Contracts page content component
 * Contains all the JSX/HTML that was previously in the page file
 * Separates presentation from routing and permission logic
 */
export function ParishionerContractsPageContent({ locale }: ParishionerContractsPageContentProps) {
  ...
  // Refresh contracts list with current filters
  ...
  }, [formData, t, createContract, refreshContracts, resetForm]);
  ...
  }, [selectedContract, formData, t, updateContract, refreshContracts]);
  ...
  }, [t, deleteContract, refreshContracts]);

  const handleEdit = useCallback((contract: ParishionerContract) => {
  ...
  ], [t, formatDate, getParishionerName, handleView, handleEdit, handleDelete]);
