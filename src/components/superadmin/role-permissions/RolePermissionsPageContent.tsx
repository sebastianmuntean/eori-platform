...
import { Role, Permission } from '../../../types/superadmin';
...
interface RolePermissionsPageContentProps {
  locale: string;
}

/**
 * Role Permissions page content component
 * Contains all the JSX/HTML that was previously in the page file
 * Separates presentation from routing and permission logic
 */
export function RolePermissionsPageContent({ locale }: RolePermissionsPageContentProps) {
  ...
  const handleOpenModal = useCallback((role: Role) => {
  ...
  const handleToggleResourcePermissions = useCallback((resource: string) => {
  ...
  }, [permissionsByResource, selectedPermissionIds]);
  ...
  }, [selectedRole, selectedPermissionIds, isSaving, fetchData]);

  const breadcrumbs = [
    { label: t('breadcrumbDashboard'), href: `/${locale}/dashboard` },
    { label: t('breadcrumbSuperadmin'), href: `/${locale}/dashboard/superadmin` },
    { label: t('rolePermissions') || 'Role Permissions' },
  ];
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        title={t('configureRolePermissions') || 'Configure Role Permissions'}
        className="mb-6"
      />
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {role.permissions.length > 0 ? (
                      ...
                      <span className="text-sm text-text-muted">{t('noPermissions') || 'No permissions'}</span>
                    )}
                  </div>
                </div>
              ))}
      ...
      <Modal
        ...
        title={`${t('configurePermissions') || 'Configure Permissions'} - ${selectedRole?.name}`}
        size="full"
      >
          ...
          <div className="border-b border-border pb-4 mb-4 px-6 pt-4 flex-shrink-0">
            <label className="flex items-center gap-2 cursor-pointer hover:bg-bg-secondary p-3 rounded-lg">
              <input
                ...
                className="w-5 h-5"
              />
              <span className="font-bold text-base text-white">{t('selectAllPermissions') || 'Select All Permissions'}</span>
            </label>
          </div>
