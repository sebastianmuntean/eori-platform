...
interface BaptismEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCancel: () => void;
...
/**
 * Modal component for editing an existing baptism event
 * Includes status field which is not available in add mode
 */
export function BaptismEditModal({
  isOpen,
  onClose,
  onCancel,
  formData,
  ...
  return (
    <FormModal
      ...
      onClose={onClose}
      onCancel={onCancel}
      title={`${t('edit') || 'Editează'} ${t('baptism') || 'Botez'}`}
      onSubmit={onSubmit}
