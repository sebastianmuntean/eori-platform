...
import { BaptismAddModal } from '@/components/events/BaptismAddModal';
import { BaptismFormData } from '@/components/events/types';
import { BaptismEditModal } from '@/components/events/BaptismEditModal';
...
/**
 * Baptisms page content component
 ...
 */
export function BaptismsPageContent({ locale }: BaptismsPageContentProps) {
  ...
  useEffect(() => {
    const params: any = {
      page: currentPage,
      pageSize: 10,
      type: 'baptism', // Filter by baptism type
  ...
  const handleUpdate = useCallback(async () => {
    ...
    // Error handling is done by the hook - error state is displayed in table card
  }, [selectedEvent, formData, updateEvent]);
  ...
  const handleConfirm = useCallback(async (id: string) => {
    ...
    if (!result) {
      ...
      console.error('Failed to confirm baptism event');
    }
  }, [confirmEvent]);
  ...
  return (
    <PageContainer>
      ...
      <BaptismsTableCard
        ...
        emptyMessage={t('noData') || 'No baptisms available'}
      />
