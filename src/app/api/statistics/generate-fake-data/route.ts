...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Fixed password for fake users: Password123!
    ...
    const results = {
      ...
      documents: 0,
      users: 0,
      createdUsers: [] as Array<{ email: string; name: string; role: string; password: string }>,
      errors: [] as string[],
    };
    ...
    // Generate users
    const createdUsers: Array<{ email: string; name: string; role: string; password: string }> = [];
    ...
    if (usersCount > 0) {
      try {
        ...
        // Hash the fixed password once for all users
        const passwordHash = await hashPassword(FAKE_USER_PASSWORD);
        ...
        for (let i = 0; i < usersCount; i++) {
          ...
          // Store user info with password for response
          createdUsers.push({
            email,
            name,
            role,
        ...
        if (usersToCreate.length > 0) {
          ...
          results.createdUsers = createdUsers;
        }
      } catch (error: any) {
    ...
    if (results.users > 0) {
      messages.push(`${results.users} utilizatori`);
      if (results.createdUsers.length > 0) {
        messages.push(`(parolă pentru toți: ${FAKE_USER_PASSWORD})`);
      }
    }
