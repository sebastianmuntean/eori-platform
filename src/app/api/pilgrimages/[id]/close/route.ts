...
/**
 * POST /api/pilgrimages/[id]/close - Close registration for a pilgrimage
 */
export async function POST(
  ...
  try {
    ...
    if (pilgrimage.status !== 'open') {
      return NextResponse.json(
        { success: false, error: 'Only open pilgrimages can be closed' },
        { status: 400 }
      );
    }
    ...
    console.error('❌ Error closing pilgrimage:', error);
    logError(error, { endpoint: '/api/pilgrimages/[id]/close', method: 'POST' });
