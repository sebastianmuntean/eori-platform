...
/**
 * POST /api/pilgrimages/[id]/publish - Publish a pilgrimage (make it open for registration)
 */
export async function POST(
  ...
  try {
    ...
    if (pilgrimage.status !== 'draft' && pilgrimage.status !== 'closed') {
      return NextResponse.json(
        { success: false, error: 'Only draft or closed pilgrimages can be published' },
        { status: 400 }
      );
    }
    ...
    logError(error, { endpoint: '/api/pilgrimages/[id]/publish', method: 'POST' });
